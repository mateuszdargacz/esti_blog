...
var nicEditorButton = bkClass.extend({construct: function (D, A, C, B) {
    ...
    this.button = new bkElement("DIV").setStyle({overflow: "hidden", zoom: 1, cursor: "pointer"}).addClass("button sprite_helper").appendTo(this.border);
...
var nicEditorSelect = bkClass.extend({construct: function (D, A, C, B) {
    ...
    this.pane = new nicEditorPane(this.items, this.ne, {width: "88px", padding: "0px", backgroundColor: "#fff"});
...
var nicXHTML = bkClass.extend({stripAttributes: ["_moz_dirty", "_moz_resizing", "_extended"], noShort: ["style", "title", "script", "textarea", "a"], cssReplace: {"font-weight:bold;": "strong", "font-style:italic;": "em"}, sizes: {1: "xx-small", 2: "x-small", 3: "small", 4: "medium", 5: "large", 6: "x-large"}, construct: function (A) {
    this.ne = A;
...
var nicUploadButton = nicEditorAdvancedButton.extend({nicURI: "http://api.imgur.com/2/upload.json", errorText: "Failed to upload image", addPane: function () {
    ...
    if (this.im) {
        this.im.setAttributes({src: D, width: (A && B.image.width) ? Math.min(A, B.image.width) : ""})
    }
}});
